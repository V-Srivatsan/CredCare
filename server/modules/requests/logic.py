...
async def get_created_requests(uid: str):
    user = await user_models.User.get(uid=uid).prefetch_related('membership__community')
    
    if not user.membership:
        raise HTTPException(status_code=404, detail="User does not belong to any community")
    
    requests = await models.Request.filter(creator=user).prefetch_related('acceptor')
        ...
        "requests": [
            {
                ...
                "acceptor": request.acceptor.name if request.acceptor else None,
                "updated_at": request.updated_at.isoformat()        
            } for request in requests
        ]
    }

async def get_accepted_requests(uid: str):
    user = await user_models.User.get(uid=uid).prefetch_related('membership__community')
...
async def complete_request(request_uid: str, user_uid: str):
