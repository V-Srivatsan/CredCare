...
@router.get('/created')
async def get_created_requests(user: str = Depends(middleware.get_verified_user)):
    return await logic.get_created_requests(user)

@router.get('/accepted')
async def get_accepted_requests(user: str = Depends(middleware.get_user)):
...
async def cancel_request(request_id: str, user: str = Depends(middleware.get_verified_user)):
...
async def complete_request(request_id: str, user: str = Depends(middleware.get_verified_user)):
